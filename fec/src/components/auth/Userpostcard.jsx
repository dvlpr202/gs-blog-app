...
const Userpostcard = ({ posts, getData }) => {
  ...
  const facebookCurl = window.location.href + "/" + posts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + posts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + posts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(posts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (posts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${posts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${posts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
        ...
        >
          <Typography variant="h6">"{posts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{posts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${posts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {posts.posted_by.name} | {posts.categories}
            </Typography>
          }
          subheader={
