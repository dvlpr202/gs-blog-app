...
const DesignPostCard = ({ designposts, getData }) => {
    const { state, dispatch } = useContext(UserContext);
  const navigate = useNavigate();
  const [likess, setLikess] = useState(
    designposts.likes.includes(state._id) ? true : false
  );
  ...
  const facebookCurl = window.location.href + "/" + designposts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + designposts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + designposts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(designposts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (designposts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${designposts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${designposts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
              image={designposts.picture}
              alt="Paella dish"
              sx={{
        ...
        >
          <Badge
            badgeContent={designposts.likes.length}
            color="primary"
            anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
          >
            <IconButton
              onClick={() => {
                handleLikes(designposts._id);
              }}
              sx={{
                color: () => {
                  if (likess === true) {
        ...
        >
          <Typography variant="h6">"{designposts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{designposts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${designposts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {designposts.posted_by.name} | {designposts.categories}
            </Typography>
          }
          subheader={
            <Typography variant="subtitle2" color="textSecondary">
              {moment(designposts.created_at).fromNow()}
            </Typography>
          }
        />
      </Card>
      ...
      >
        ...
        <DialogActions>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              addComment(e.target[0].value, designposts._id);
            }}
          >
            <TextField
              sx={{
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => delPost(designposts._id)}
            autoFocus
            sx={{
              color: "red",
            }}
