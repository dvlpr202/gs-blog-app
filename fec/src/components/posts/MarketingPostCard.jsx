...
const MarketingPostCard = ({ marketingposts, getData }) => {
    const { state, dispatch } = useContext(UserContext);
  const navigate = useNavigate();
  const [likess, setLikess] = useState(
    marketingposts.likes.includes(state._id) ? true : false
  );
  ...
  const facebookCurl = window.location.href + "/" + marketingposts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + marketingposts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + marketingposts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(marketingposts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (marketingposts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${marketingposts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${marketingposts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
              image={marketingposts.picture}
              alt="Paella dish"
              sx={{
        ...
        >
          <Badge
            badgeContent={marketingposts.likes.length}
            color="primary"
            anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
          >
            <IconButton
              onClick={() => {
                handleLikes(marketingposts._id);
              }}
              sx={{
                color: () => {
                  if (likess === true) {
        ...
        >
          <Typography variant="h6">"{marketingposts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{marketingposts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${marketingposts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {marketingposts.posted_by.name} | {marketingposts.categories}
            </Typography>
          }
          subheader={
            <Typography variant="subtitle2" color="textSecondary">
              {moment(marketingposts.created_at).fromNow()}
            </Typography>
          }
        />
      </Card>
      ...
      >
        ...
        <DialogActions>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              addComment(e.target[0].value, marketingposts._id);
            }}
          >
            <TextField
              sx={{
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => delPost(marketingposts._id)}
            autoFocus
            sx={{
              color: "red",
            }}
