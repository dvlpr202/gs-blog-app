...
const MobilePostCard = ({ mobileposts, getData }) => {
    const { state, dispatch } = useContext(UserContext);
  const navigate = useNavigate();
  const [likess, setLikess] = useState(
    mobileposts.likes.includes(state._id) ? true : false
  );
  ...
  const facebookCurl = window.location.href + "/" + mobileposts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + mobileposts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + mobileposts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(mobileposts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (mobileposts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${mobileposts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${mobileposts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
              image={mobileposts.picture}
              alt="Paella dish"
              sx={{
        ...
        >
          <Badge
            badgeContent={mobileposts.likes.length}
            color="primary"
            anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
          >
            <IconButton
              onClick={() => {
                handleLikes(mobileposts._id);
              }}
              sx={{
                color: () => {
                  if (likess === true) {
        ...
        >
          <Typography variant="h6">"{mobileposts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{mobileposts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${mobileposts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {mobileposts.posted_by.name} | {mobileposts.categories}
            </Typography>
          }
          subheader={
            <Typography variant="subtitle2" color="textSecondary">
              {moment(mobileposts.created_at).fromNow()}
            </Typography>
          }
        />
      </Card>
      ...
      >
        ...
        <DialogActions>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              addComment(e.target[0].value, mobileposts._id);
            }}
          >
            <TextField
              sx={{
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => delPost(mobileposts._id)}
            autoFocus
            sx={{
              color: "red",
            }}
