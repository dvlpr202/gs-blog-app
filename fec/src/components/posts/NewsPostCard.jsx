...
const NewsPostCard = ({ newsposts, getData }) => {
    const { state, dispatch } = useContext(UserContext);
  const navigate = useNavigate();
  const [likess, setLikess] = useState(
    newsposts.likes.includes(state._id) ? true : false
  );
  ...
  const facebookCurl = window.location.href + "/" + newsposts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + newsposts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + newsposts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(newsposts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (newsposts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${newsposts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${newsposts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
              image={newsposts.picture}
              alt="Paella dish"
              sx={{
        ...
        >
          <Badge
            badgeContent={newsposts.likes.length}
            color="primary"
            anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
          >
            <IconButton
              onClick={() => {
                handleLikes(newsposts._id);
              }}
              sx={{
                color: () => {
                  if (likess === true) {
        ...
        >
          <Typography variant="h6">"{newsposts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{newsposts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${newsposts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {newsposts.posted_by.name} | {newsposts.categories}
            </Typography>
          }
          subheader={
            <Typography variant="subtitle2" color="textSecondary">
              {moment(newsposts.created_at).fromNow()}
            </Typography>
          }
        />
      </Card>
      ...
      >
        ...
        <DialogActions>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              addComment(e.target[0].value, newsposts._id);
            }}
          >
            <TextField
              sx={{
