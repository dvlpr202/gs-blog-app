...
const RecentPostsCard = ({ reposts, getData }) => {
  const { state, dispatch } = useContext(UserContext);
  const navigate = useNavigate();
  const [likess, setLikess] = useState(
    reposts.likes.includes(state._id) ? true : false
  );
  ...
  const facebookCurl = window.location.href + "/" + reposts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + reposts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + reposts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(reposts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (reposts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${reposts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${reposts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
              image={reposts.picture}
              alt="Paella dish"
              sx={{
        ...
        >
          <Badge
            badgeContent={reposts.likes.length}
            color="primary"
            anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
          >
            <IconButton
              onClick={() => {
                handleLikes(reposts._id);
              }}
              sx={{
                color: () => {
                  if (likess === true) {
        ...
        >
          <Typography variant="h6">"{reposts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{reposts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${reposts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {reposts.posted_by.name} | {reposts.categories}
            </Typography>
          }
          subheader={
            <Typography variant="subtitle2" color="textSecondary">
              {moment(reposts.created_at).fromNow()}
            </Typography>
          }
        />
      </Card>
      ...
      >
        ...
        <DialogActions>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              addComment(e.target[0].value, reposts._id);
            }}
          >
            <TextField
              sx={{
