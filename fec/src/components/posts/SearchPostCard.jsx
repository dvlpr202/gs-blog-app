...
const SearchPostCard = ({ searchposts, getData }) => {
  const { state, dispatch } = useContext(UserContext);
  const navigate = useNavigate();
  const [likess, setLikess] = useState(
    searchposts.likes.includes(state._id) ? true : false
  );
  ...
  const facebookCurl = window.location.href + "/" + searchposts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + searchposts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + searchposts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(searchposts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (searchposts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${searchposts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${searchposts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
              image={searchposts.picture}
              alt="Paella dish"
              sx={{
        ...
        >
          <Badge
            badgeContent={searchposts.likes.length}
            color="primary"
            anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
          >
            <IconButton
              onClick={() => {
                handleLikes(searchposts._id);
              }}
              sx={{
                color: () => {
                  if (likess === true) {
        ...
        >
          <Typography variant="h6">"{searchposts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{searchposts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${searchposts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {searchposts.posted_by.name} | {searchposts.categories}
            </Typography>
          }
          subheader={
            <Typography variant="subtitle2" color="textSecondary">
              {moment(searchposts.created_at).fromNow()}
            </Typography>
          }
        />
      </Card>
      ...
      >
        ...
        <DialogActions>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              addComment(e.target[0].value, searchposts._id);
            }}
          >
            <TextField
              sx={{
      ...
      >
        ...
        <DialogActions>
          ...
          <Button
            onClick={() => delPost(searchposts._id)}
            autoFocus
            sx={{
              color: "red",
            }}
