...
const WebPostCard = ({ webposts, getData }) => {
  const { state, dispatch } = useContext(UserContext);
  const navigate = useNavigate();
  const [likess, setLikess] = useState(
    webposts.likes.includes(state._id) ? true : false
  );
  ...
  const facebookCurl = window.location.href + "/" + webposts._id;
  const facebookLink = `https://facebook.com/sharer.php?${facebookCurl}`;
  const twitterCurl = window.location.href + "/" + webposts._id;
  const twitterLink = `https://twitter.com/intent/tweet?${twitterCurl}`;
  const whatsappCurl = window.location.href + "/" + webposts._id;
  ...
  const getComments = async () => {
    const getAllComments = await showAllComments(webposts._id);
    setCommentsarr(getAllComments.data);
    // console.log(getAllComments.data);
  };

  const editVal = () => {
    if (webposts.posted_by._id == state._id) {
      return (
        <IconButton component={Link} to={`/updatepost/${webposts._id}`}>
          <AutoFixHighIcon />
        </IconButton>
      );
    }
  ...
  return (
    <>
      ...
      >
        <Box component={Link} to={`/compost/${webposts._id}`}>
          <Card elevation={0}>
            <CardMedia
              component="img"
              height="280"
              image={webposts.picture}
              alt="Paella dish"
              sx={{
        ...
        >
          <Badge
            badgeContent={webposts.likes.length}
            color="primary"
            anchorOrigin={{ vertical: "bottom", horizontal: "left" }}
          >
            <IconButton
              onClick={() => {
                handleLikes(webposts._id);
              }}
              sx={{
                color: () => {
                  if (likess === true) {
        ...
        >
          <Typography variant="h6">"{webposts.sdesc}"</Typography>
        </CardContent>
        <CardHeader
          avatar={<Avatar>{webposts.posted_by.name[0]}</Avatar>}
          action={
            <Button
              component={Link}
              to={`/compost/${webposts._id}`}
          ...
          title={
            <Typography variant="subtitle" color="textSecondary">
              {webposts.posted_by.name} | {webposts.categories}
            </Typography>
          }
          subheader={
            <Typography variant="subtitle2" color="textSecondary">
              {moment(webposts.created_at).fromNow()}
            </Typography>
          }
        />
      </Card>
      ...
      >
        ...
        <DialogActions>
          <form
            ...
            onSubmit={(e) => {
              e.preventDefault();
              addComment(e.target[0].value, webposts._id);
            }}
          >
            <TextField
              sx={{
